...
resultCons <- phyloP(tm, msa, basewise=TRUE)
#resultAcc <- phyloP(tm, msa, basewise=TRUE, mode="ACC")
#resultConAcc <- phyloP(tm, msa, basewise=TRUE, mode="CONACC")
temp <- phyloP(tm, msa, basewise=TRUE, mode="CONACC", output.file="test.out")
smallMsa <- sub.msa(msa, start.col=10900, end.col=12000)
#temp <- phyloP(tm, smallMsa, method="SPH")
temp <- phyloP(tm, smallMsa, method="LRT", subtree="mm9-rn4", basewise=TRUE, output.file="test.out")
temp <- phyloP(tm, smallMsa, method="LRT", branches=c("mm9", "rn4"), basewise=TRUE)
gff <- read.gff("gencode.ENr334.gff")
temp <- phyloP(tm, msa, method="LRT", gff=gff)
phyloP(tm, msa, method="LRT", output.file.only="test.out", basewise=TRUE, chrom="chr1")
