...
Genome Browser-style names (e.g., ``sol1''), but it will be more convenient
to use common names here.  The sequence names in the alignment must match
those in the tree.  In addition, the sequence name used for the gene
annotations must match the name of the corresponding sequence in the
...
wholeChrom <- feat(seq="tomato", src=".", feature="all",
                  start=align$offset, end=122080)

# annotate intergenic regions
intergenic.feats <- inverse.feat(feats, region.bounds=wholeChrom)
intergenic.feats$feature <- "intergenic"
feats <- rbind.feat(feats, intergenic.feats)
@ 
Next we will estimate a neutral model from fourfold degenerate (4D) sites
in coding regions, using phyloFit with the prefefined tree topology and the
...
plot.track(list(geneTrack, consElTrack, phastConsScoreTrack, phyloPTrack),
           xlim=c(60000, 68000), cex.labels=1.5, cex.axis=1.5, cex.lab=2.0)

@

Observe that the conserved elements appear to track the coding exons
reasonably well, but they also contain some noncoding regions.  The
(smoothed) phyloP scores are reasonably consistent with the phastCons
...
coding.feats <- feats[feats$feature=="CDS",]
# obtain elements that overlap coding.feats by at least 50 percent
...
From this plot we see that the coding elements are clearly longer, on
average---as would be expected.   
...
<<coverage-composition,width=8,height=8>>=
par(mfrow=c(2, 2), cex.main=2.0, cex.lab=2.0, cex.axis=2.0, mar=c(5,5,4,2))
...
plot.new()
legend(x="center", legend=enrich$type, fill=col, cex=2.5)
...
Here we see that coding regions are enriched more than fourfold in
