...
First we will show an example of conservation analysis in a non-model organism,
...
# are plottable
plotData <- getPlottableResults(list(feats=feats[feats$feature=="CDS",],
                                     phastCons=pc,
                                     phyloP=pp))
names(plotData)
...
<<rarAnalysis1>>=
require("rphast")
elements <- read.gff("/home/melissa/rphast/chr22-elements.bed")
...
barplot(lr, density=1)
pwm <- makePWM(t(m))
seqLogo(pwm)
barplot(lr, density=1, add=TRUE)
...
specificity <- coverage.gff(correct, predicted, not=c(TRUE, TRUE), region.bounds=wholeRegion)/
