...
alignment which excludes mouse and rat.  Then we could examine the full
alignments for signs of acceleration in mouse-rat in the identified
elements.  However, to keep the example simple we will simply make use of
...
call.  Some RPHAST functions do indeed change the value of arguments that
are passed as pointers, but this behavior is always documented in the help
files.
...
@ 
Notice that the two distributions appear fairly similar in the density
plots, except that a slightly heavier right tail is evident for the
observed elements (in black).  This excess of high likelihood ratios for the
observed elements shows up clearly in the Q-Q plot.
...
candidate ``RAR''s (Rodent Accelerated Regions).  We will write them to a
file, which could be used, say, to display them as a custom track in the
UCSC Genome Browser, or to perform an analysis using Galaxy.  Notice that
we have to change the sequence names to allow them to be recognized by the
UCSC Genome Browser.
<<rarAnalysis11>>=
...
test based on other models available in PHAST, but not supported by
off-the-shelf functions for likelihood ratio tests.  Below we show how the
...
length(para.sigIdx)
@ 
We can see that this method appears to be somewhat less conservative---it
identifies nearly five times as many elements.  This may indicate that the
parametric model is too simple, for example, by failing to allow for rate
variation among sites, indels, or missing data.  
