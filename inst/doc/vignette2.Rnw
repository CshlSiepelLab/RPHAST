...
Now, read the alignment.  For simplicity we will use a a subset of the full species set.
...
align <- read.msa("chr22.maf", seqnames=seqnames)

@ 

In this type of analysis it is usually necessary to apply some filters to the data
to control the rate of false positives.  In a real analysis we would suggest filters
synteny, repeat, and missing data filters.  Here we will implement a missing data filter,
which in this case, is probably the most important.  We want to only analyze regions
which have data in both mouse and rat, as well a total of at least four species.
<<>>=

hasMouseRat <- informative.regions.msa(align, 2, c("mm9", "rn4"))
hasAtLeastFour <- informative.regions.msa(align, 4)
informativeElements <- coverage.feat(elements, hasMouseRat, hasAtLeastFour, get.feats=TRUE)

elementAlign <- extract.feature.msa(align, informativeElements)
...
plot.track(list(feat.track(nonpara.sigFeats, "RAR"),
                feat.track(elements, "Conserved elements")))
...
# let's just do a subset of the elements for demonstration
sampleSize <- 50
for (i in 1:sampleSize) {
  obs.phyloP[i] <- branchTest(sub.msa(align, start.col=splitElements[i,"start"],
...
for (i in 1:nrep) {
  ...
  sim.lr[i] <- branchTest(simMsa, neutralMod, "mm9-rn4")
}
