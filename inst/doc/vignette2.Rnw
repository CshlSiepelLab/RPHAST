...
<<qqPlotAndDensity,fig=TRUE,prefix=FALSE,width=6,height=8>>=
...
The plots can be seen in Figure \ref{fig:qqplot}.  
...
the ``ape'' package to plot the estimated trees side-by-side (Figure 
\ref{fig:ape-plot}).   We can see
that the RARs do show a pronounced evolutionary speedup in the rodents.

<<apePlot,fig=TRUE,prefix=FALSE>>=
...
\begin{figure}
  \begin{center}
    \includegraphics[width=6in,height=8in,angle=0]{qqPlotAndDensity}
  \end{center}
  \caption{On top, Q-Q plot for observed and simulated likelihood ratios.  On bottom, distributions of observed (red) and simulated (black) values.}
  \label{fig:qqplot}
\end{figure}

\begin{figure}
  \begin{center}
    \includegraphics[width=6in,height=4.5in,angle=0]{apePlot}
  \end{center}
  \caption{Trees estimated from conserved elements (left) and predicted rodent accelerated regions (right), plotted by the ape package.}
  \label{fig:ape-plot}
\end{figure}
\end{document}
