...
<<rarAnalysis2>>=
splitLength <- 50
splitElements <- split.feat(elements, max.length=splitLength)
splitElements <- splitElements[splitElements$end - splitElements$start + 1 == splitLength,]

@ 

Now, read the alignment.  By using the features argument we get an un-ordered
...
if (!file.exists("chr22.maf")) {
  if (!file.exists("chr22.maf.zip"))
    download.file("http://compgen.bscb.cornell.edu/phast/chr22.maf.zip",
                  "chr22.maf.zip", method="auto")
  unzip("chr22.maf.zip", "chr22.maf")
}
elementAlign <- read.msa("chr22.maf", seqnames=seqnames, features=elements)
@ 
